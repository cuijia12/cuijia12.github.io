...
- T5L 在线下载与 T5L51 更新
- 单文件 EXE，无需安装 Python
- Agent API：供 Codex、脚本等智能体打开/关闭串口、发送数据、读取最新应答及启动 T5L 下载
...
## Agent API（Codex / 自动化调用）

程序运行后仅在本机 `127.0.0.1:18765` 提供带 Token 验证的 HTTP JSON 接口，不对局域网或互联网开放。

- 可打开/关闭指定 COM 口，配置线路参数并发送 HEX 或字符数据。
- `POST /api/serial/receive` 可读取指定 COM 口的最新接收数据，返回 HEX、字符、字节数和时间。
- 可启动、停止和查询 T5L 在线下载；接口与主界面共用串口会话，不会重复占用端口。
- 接收缓存与界面通讯记录独立，接口清空缓存不会删除界面日志。

完整 JSON 参数和 PowerShell/命令行示例见 [Agent API 文档](./src/AGENT_API.md)。
